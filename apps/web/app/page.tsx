...
const revenueNextAction = (status: string) => status === 'ESCALATED' ? 'Manual decision' : nextAction(status);
const payoutSource = (payoutId: string) => payoutId.includes('pout_seed_rx_demo') ? 'PROVIDER-BACKED TEST' : (payoutId.startsWith('pout_sim_') || payoutId.includes('demo')) ? 'SYNTHETIC' : 'PROVIDER EVENT';
...
export default function Dashboard() {
  ...
  const [payoutMetrics, setPayoutMetrics] = useState<Metrics | null>(null);
  ...
  const load = async (token = authToken, requestedPage = incidentPage.page, filters = { search, statusFilter, reviewOnly }) => {
    ...
    try {
      ...
      const [sessionResponse, incidentResponse, metricResponse, batchResponse, policyResponse, operationsResponse, revenueResponse, experimentResponse, revenueOperationsResponse, revenueMetricResponse] = await Promise.all([apiFetch('/session', { cache: 'no-store' }, token), apiFetch(`/incidents?${incidentQuery}`, { cache: 'no-store' }, token), apiFetch('/metrics', { cache: 'no-store' }, token), apiFetch('/batches', { cache: 'no-store' }, token), apiFetch('/policies', { cache: 'no-store' }, token), apiFetch('/operations', { cache: 'no-store' }, token), apiFetch('/revenue/incidents?page=1&pageSize=100', { cache: 'no-store' }, token), apiFetch('/revenue/experiments', { cache: 'no-store' }, token), apiFetch('/revenue/operations', { cache: 'no-store' }, token), apiFetch('/revenue/metrics', { cache: 'no-store' }, token)]);
      const responses = [sessionResponse, incidentResponse, metricResponse, batchResponse, policyResponse, operationsResponse, revenueResponse, experimentResponse, revenueOperationsResponse, revenueMetricResponse];
      if (responses.some(response => response.status === 401)) { setError('AUTH_REQUIRED'); setSession(null); return; }
      if ([sessionResponse, incidentResponse, metricResponse, batchResponse, policyResponse, operationsResponse].some(response => !response.ok)) throw new Error('API unavailable');
      const [sessionData, incidentData, metricData, batchData, policyData, operationsData] = await Promise.all([sessionResponse.json(), incidentResponse.json(), metricResponse.json(), batchResponse.json(), policyResponse.json(), operationsResponse.json()]);
      setSession(sessionData);
      setIncidents(incidentData.items); setIncidentPage({ total: incidentData.total, page: incidentData.page, pageSize: incidentData.pageSize, totalPages: incidentData.totalPages }); setPayoutMetrics(metricData); setBatches(batchData); setPolicy(policyData); setOperations(operationsData);
      if ([revenueResponse, experimentResponse, revenueOperationsResponse, revenueMetricResponse].every(response => response.ok)) {
        const [revenueData, experimentData, revenueOperationsData, revenueMetricData] = await Promise.all([revenueResponse.json(), experimentResponse.json(), revenueOperationsResponse.json(), revenueMetricResponse.json()]);
        setRevenueIncidents(revenueData.items); setRevenueExperiments(experimentData); setRevenueConfiguration(revenueOperationsData); setRevenueMetrics(revenueMetricData); setRevenueError('');
      ...
      } else setRevenueError('Inbound revenue data is temporarily unavailable; payout operations remain usable.');
  ...
  const open = async (id: string) => { setReviewError(''); const response = await apiFetch(`/incidents/${id}`); if (response.ok) setDetail(await response.json()); };
  const review = async (approved: boolean) => {
    if (!detail || !can(session, 'OPERATOR') || reviewBusy) return;
    setReviewBusy(true); setReviewError('');
    try {
      const response = await apiFetch(`/incidents/${detail.id}/${approved ? 'approve' : 'reject'}`, { method: 'POST' });
      if (!response.ok) {
        const body = await response.json().catch(() => null) as { message?: string | string[] } | null;
        const rawMessage = Array.isArray(body?.message) ? body.message.join(' ') : body?.message;
        const message = response.status === 409 && rawMessage?.includes('Maker-checker')
          ? 'Approval blocked by maker-checker control. Sign out and reconnect with the other operator or administrator credential; the person who recorded the remediation cannot approve it.'
          : rawMessage || 'The review decision could not be recorded. Refresh the incident and try again.';
        setReviewError(message); setNotice(message); return;
      }
      await open(detail.id); await load();
      setNotice(approved ? 'Retry approved and queued for policy-controlled execution.' : 'Incident rejected and stopped.');
    } catch {
      const message = 'The review decision could not reach the API. Check system readiness and try again.';
      setReviewError(message); setNotice(message);
  ...
  const createBatch = async () => { if (!incidents.length || !can(session, 'OPERATOR')) return; const response = await apiFetch('/batches', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: `Operator filtered page ${new Date().toLocaleString()}`, incidentIds: incidents.map(incident => incident.id) }) }); const body = await response.json().catch(() => null); if (response.ok) { const batch = body as Batch; setNotice(`Created ${batch.name}`); setSelectedBatch(batch); await load(); } else setNotice((body as { message?: string } | null)?.message || 'The current page could not be saved as evidence.'); };
  const savePolicy = async () => { if (!policy || !can(session, 'ADMIN')) return; const response = await apiFetch('/policies', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(policy) }); const body = await response.json().catch(() => null); if (response.ok) { setPolicy(body); setNotice('Policy version activated.'); } else setNotice((body as { message?: string } | null)?.message || 'The policy could not be updated.'); };
  const reconcile = async () => {
    if (!can(session, 'ADMIN') || reconcileBusy) return;
    setReconcileBusy(true);
    try {
      const response = await apiFetch('/reconcile', { method: 'POST' });
      const body = await response.json().catch(() => null) as { scanned?: number; reconciled?: number; pending?: number; failures?: number; message?: string | string[] } | null;
      if (!response.ok) {
        const message = Array.isArray(body?.message) ? body.message.join(' ') : body?.message;
        setNotice(message || 'Reconciliation could not be completed.');
        return;
      }
      setNotice(`Reconciliation scanned ${body?.scanned ?? 0} incident(s): ${body?.reconciled ?? 0} terminal update(s), ${body?.pending ?? 0} still pending, ${body?.failures ?? 0} lookup failure(s).`);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      try {
        ...
        const [batchResponse, metricResponse] = await Promise.all([apiFetch(`/batches/${demoRun.batch.id}`), apiFetch('/metrics')]);
        ...
        if (metricResponse.ok) setPayoutMetrics(await metricResponse.json());
      } catch {}
    }, 1_500);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      try {
        const [incidentResponse, batchResponse, metricResponse] = await Promise.all([
  ...
  useEffect(() => {
    if (view !== 'incidents' || !detail || ['RECOVERED', 'FAILED', 'STOPPED', 'REVERSED'].includes(detail.status)) return;
    const timer = window.setInterval(async () => {
      try {
        const [incidentResponse, metricResponse, batchResponse] = await Promise.all([apiFetch(`/incidents/${detail.id}`), apiFetch('/metrics'), apiFetch('/batches')]);
        ...
        if (metricResponse.ok) setPayoutMetrics(await metricResponse.json());
        if (batchResponse.ok) {
          const refreshedBatches = await batchResponse.json();
          setBatches(refreshedBatches);
          setSelectedBatch(current => refreshedBatches.find((batch: Batch) => batch.id === current?.id) ?? refreshedBatches[0] ?? null);
        }
      } catch {}
    }, 2_000);
    return () => window.clearInterval(timer);
  }, [view, detail?.id, detail?.status]);

  const pageMetrics = useMemo<Metrics>(() => {
    ...
    return {
      valueAtRiskPaise: atRisk, openValueAtRiskPaise: incidents.filter(incident => !['RECOVERED', 'STOPPED', 'FAILED', 'REVERSED'].includes(incident.status)).reduce((sum, incident) => sum + incident.amountPaise, 0), recoveredValuePaise: recovered, recoveryRate: atRisk ? recovered / atRisk : 0,
  ...
  const accountMetrics = payoutMetrics ?? pageMetrics;
  const metrics = view === 'batches' ? selectedBatch?.liveMetrics ?? selectedBatch?.metrics ?? accountMetrics : accountMetrics;
  ...
  return <><a className="skip-link" href="#main-content">Skip to main content</a><div className={`app-shell ${error ? 'is-auth' : ''}`}>
    ...
    {view === 'revenue' && <><p className="eyebrow portfolio-label">RECOVERY PORTFOLIO · LIVE ACCOUNT TOTALS</p><section className="metrics primary-metrics" aria-label="Combined recovery portfolio summary"><Metric label="Total recovered" value={money(accountMetrics.recoveredValuePaise + (revenueMetrics?.recoveredValuePaise ?? 0))} accent="green" /><Metric label="Payouts recovered" value={money(accountMetrics.recoveredValuePaise)} accent="green" /><Metric label="Inbound recovered" value={money(revenueMetrics?.recoveredValuePaise ?? 0)} accent="green" /><Metric label="Open portfolio risk" value={money((accountMetrics.openValueAtRiskPaise ?? accountMetrics.valueAtRiskPaise) + (revenueMetrics?.openValueAtRiskPaise ?? 0))} accent="orange" /></section></>}
    ...
    {!error && view === 'operations' && <OperationsWorkspace operations={operations} canAdmin={can(session, 'ADMIN')} reconcileBusy={reconcileBusy} onRefresh={() => load()} onReconcile={reconcile} />}
    </>}
...
function RevenueIncidentDetail({ detail, canOperate, onReview }: { detail: RevenueIncident; canOperate: boolean; onReview: (approved: boolean) => void }) {
  ...
  return <><div className="panel-head"><div><p className="eyebrow">INBOUND PAYMENT INCIDENT</p><h2>{detail.sourcePaymentId}</h2></div><span className={`badge ${tone(detail.status)}`}>{detail.status.replaceAll('_', ' ')}</span></div><div className="facts"><span>Revenue at risk<strong>{money(detail.amountPaise)}</strong></span><span>Payment method<strong>{detail.paymentMethod}</strong></span><span>Failure<strong>{detail.failureCode}</strong></span><span>Contact consent<strong>{detail.consentToContact ? 'PRESENT' : 'ABSENT'}</strong></span></div>
    ...
    {detail.status === 'ESCALATED' && <div className="actions sticky-action"><span><strong>Manual investigation required</strong><small>RecoveryOS will not execute a collection action for this exception. After reviewing it outside the automated workflow, acknowledge and stop the case here.</small></span><button className="danger" disabled={!canOperate} title={!canOperate ? 'Operator access required' : ''} onClick={() => onReview(false)}>Acknowledge and stop</button></div>}
...
function BatchWorkspace({ batches, selected, canCreate, onSelect, onCreate, onDownload }: { batches: Batch[]; selected: Batch | null; canCreate: boolean; onSelect: (batch: Batch) => void; onCreate: () => void; onDownload: (batch: Batch, format: 'csv' | 'json') => void }) {
  const currentMetrics = selected?.liveMetrics ?? selected?.metrics;
  return <section className="workspace"><div className="panel batch-list"><div className="panel-head"><div><p className="eyebrow">EVIDENCE LEDGER</p><h2>Evaluation batches</h2><p>Immutable snapshots with current linked outcomes</p></div><button disabled={!canCreate} title={!canCreate ? 'Operator access required' : 'Saves only incidents visible on the current filtered page'} onClick={onCreate}>Create from current page</button></div>{batches.length ? batches.map((batch, index) => { const outcome = batch.liveMetrics ?? batch.metrics; return <button key={batch.id} className={`batch-card ${selected?.id === batch.id ? 'selected' : ''}`} onClick={() => onSelect(batch)}><span><span className={`badge ${index === 0 ? 'good' : 'neutral'}`}>{index === 0 ? 'CURRENT' : 'ARCHIVED'}</span><strong>{batch.name}</strong><small>{new Date(batch.startedAt).toLocaleString()} · {batch.cohortSize} cases · snapshot immutable</small></span><strong>Recovered {money(outcome.recoveredValuePaise)}</strong></button>; }) : <div className="empty">No batches yet. Create one from the current incident queue.</div>}</div><aside className="panel batch-detail">{selected && currentMetrics ? <><div className="panel-head"><div><p className="eyebrow">CURRENT LINKED OUTCOMES</p><h2>{selected.name}</h2></div><span className="badge good">LIVE TOTALS</span></div><div className="batch-stats expanded"><span>Gross recovered<strong>{money(currentMetrics.recoveredValuePaise)}</strong></span><span>Eligible recovered<strong>{money(currentMetrics.recoveredEligibleValuePaise)}</strong></span><span>Eligible recovery<strong>{(currentMetrics.eligibleRecoveryRate * 100).toFixed(1)}%</strong></span><span>Open risk<strong>{money(currentMetrics.openValueAtRiskPaise ?? currentMetrics.pendingRecoveryValuePaise)}</strong></span><span>Protected value<strong>{money(currentMetrics.protectedValuePaise)}</strong></span><span>Recorded interventions<strong>{currentMetrics.manualInterventions}</strong></span></div><div className="metric-note">These cards are recalculated from the incidents linked to this batch, so recovered, stopped, processing and review states stay current. The creation-time snapshot and exports remain immutable.</div><details className="technical-details"><summary>Compare immutable creation-time snapshot</summary><div className="experiment-meta"><span>Recovered then <strong>{money(selected.metrics.recoveredValuePaise)}</strong></span><span>Pending then <strong>{money(selected.metrics.pendingRecoveryValuePaise)}</strong></span><span>Open cases then <strong>{selected.metrics.unresolvedIncidents}</strong></span><span>Captured <strong>{selected.completedAt ? new Date(selected.completedAt).toLocaleString() : 'legacy'}</strong></span></div></details>{selected.immutable && <details className="technical-details"><summary>Version and cohort fingerprint</summary><div className="experiment-meta"><span>Policy <strong>{selected.policyVersion}</strong></span><span>Model <strong>{selected.modelRef}</strong></span><span>Prompt <strong>{selected.promptVersion}</strong></span><span>Fingerprint <strong>{selected.cohortFingerprint?.slice(0, 12)}…</strong></span></div></details>}<div className="distribution"><p className="eyebrow">CURRENT OUTCOME DISTRIBUTION</p>{Object.entries(currentMetrics.statusDistribution).map(([status, count]) => <div key={status}><span>{status.replaceAll('_', ' ')}</span><strong>{count}</strong></div>)}</div><div className="actions"><button className="approve" onClick={() => onDownload(selected, 'csv')}>Download frozen CSV</button><button onClick={() => onDownload(selected, 'json')}>Download frozen JSON</button></div></> : <div className="empty">Select a batch to inspect its evidence.</div>}</aside></section>;
}
...
function OperationsWorkspace({ operations, canAdmin, reconcileBusy, onRefresh, onReconcile }: { operations: Operations | null; canAdmin: boolean; reconcileBusy: boolean; onRefresh: () => void; onReconcile: () => void }) {
  ...
  return <section className="panel operations">
    <div className="panel-head">
      ...
      <div className="actions compact"><span className={`badge ${operations.status === 'ready' ? 'good' : 'warn'}`}>{operations.status.toUpperCase()}</span><button disabled={!canAdmin || reconcileBusy} title={!canAdmin ? 'Administrator access required' : 'Check every processing or execution-unknown payout against Razorpay'} onClick={onReconcile}>{reconcileBusy ? 'Reconciling…' : 'Reconcile payouts'}</button><button onClick={onRefresh}>Refresh</button></div>
    </div>
...
function IncidentDetail({ detail, session, canOperate, canAdmin, reviewBusy, reconcileBusy, actionError, onReview, onRemediate, onReconcile }: { detail: Detail; session: Session | null; canOperate: boolean; canAdmin: boolean; reviewBusy: boolean; reconcileBusy: boolean; actionError: string; onReview: (approved: boolean) => void; onRemediate: (beneficiaryRef: string, note: string) => void; onReconcile: () => void }) {
  ...
  const approvalTask = detail.reviewTasks.find(task => task.kind === 'RETRY_APPROVAL' && task.status === 'OPEN');
  const remediationActor = typeof approvalTask?.remediationJson?.remediatedBy === 'string' ? approvalTask.remediationJson.remediatedBy : '';
  const sameActor = Boolean(session?.id && remediationActor === session.id);
  const checkerRole = session?.role === 'ADMIN' ? 'operator' : 'administrator';
  return <><div className="panel-head"><div><p className="eyebrow">PAYOUT INCIDENT DETAILS</p><h2>{detail.razorpayPayoutId}</h2></div><span className={`badge ${tone(detail.status)}`}>{detail.status.replaceAll('_', ' ')}</span></div><div className="facts"><span>Amount <strong>{money(detail.amountPaise)}</strong></span><span>Attempts <strong>{detail.attempts}</strong></span><span>Reason <strong>{detail.currentReason || '—'}</strong></span></div>{analysis && <section className="evidence"><div className="evidence-head"><p className="eyebrow">{deterministic ? 'DETERMINISTIC ADVISORY' : 'AI ADVISORY'}</p><span className="badge neutral">{analysisRecord?.modelRef}</span></div><strong>{analysis.category.replaceAll('_', ' ')}</strong><p>{analysis.evidenceSummary}</p><small>Confidence {Math.round(analysis.confidence * 100)}% · Proposed {analysis.recommendedAction} · Prompt {analysisRecord?.promptVersion}</small></section>}{decision && <section className="evidence policy"><p className="eyebrow">POLICY DECISION</p><strong>{decision.finalDecision.replaceAll('_', ' ')}</strong><p>{decision.reasonsJson.join(' ')}</p></section>}
    {detail.status === 'APPROVAL_REQUIRED' && <div className="actions sticky-action"><span><strong>{sameActor ? 'A second operator must approve this retry' : 'Bounded retry needs approval'}</strong><small>{sameActor ? `You recorded this remediation. Sign out and reconnect with the ${checkerRole} credential to approve it.` : 'Review the advisory and policy evidence before deciding.'}</small>{actionError && <small className="action-error" role="alert">{actionError}</small>}</span><button className="approve" disabled={!canOperate || reviewBusy || sameActor} title={!canOperate ? 'Operator access required' : sameActor ? 'Maker-checker control requires a different actor' : ''} onClick={() => onReview(true)}>{reviewBusy ? 'Approving…' : 'Approve retry'}</button><button className="danger" disabled={!canOperate || reviewBusy} title={!canOperate ? 'Operator access required' : ''} onClick={() => onReview(false)}>{reviewBusy ? 'Recording…' : 'Reject'}</button></div>}
    {detail.status === 'ESCALATE' && <section className="remediation"><p className="eyebrow">REMEDIATION REQUIRED · RETRY IS BLOCKED</p><p>An escalation cannot be approved directly. Record a validated replacement Razorpay fund account; a separate actor must then approve the new retry task.</p><label>Replacement Razorpay fund account ID<input disabled={!canOperate} placeholder="fa_..." value={beneficiaryRef} onChange={event => setBeneficiaryRef(event.target.value)} /></label><label>Verification evidence and remediation note<textarea disabled={!canOperate} value={remediationNote} onChange={event => setRemediationNote(event.target.value)} /></label><div className="actions compact"><button className="approve" disabled={!canOperate || !/^fa_[A-Za-z0-9]+$/.test(beneficiaryRef.trim()) || remediationNote.trim().length < 10} onClick={() => onRemediate(beneficiaryRef, remediationNote)}>Record remediation</button><button className="danger" disabled={!canOperate} onClick={() => onReview(false)}>Reject and stop</button></div>{!canOperate && <p className="access-note">Operator access is required to record remediation.</p>}</section>}
    {['PROCESSING', 'EXECUTION_UNKNOWN'].includes(detail.status) && <div className="actions sticky-action"><span><strong>Provider confirmation required</strong><small>No fresh payout will be created while the outcome remains ambiguous. Reconciliation checks every open payout and records only confirmed terminal states.</small></span><button className="approve" disabled={!canAdmin || reconcileBusy} title={!canAdmin ? 'Administrator access required' : ''} onClick={onReconcile}>{reconcileBusy ? 'Reconciling…' : 'Reconcile now'}</button></div>}
    <details className="timeline technical-details"><summary>View complete audit timeline</summary>{detail.auditEvents.map(event => <div className="event" key={event.id}><i></i><div><strong>{event.eventType.replaceAll('_', ' ')}</strong><p>{event.rationale}</p><small>{new Date(event.createdAt).toLocaleString()}</small></div></div>)}</details></>;
}
