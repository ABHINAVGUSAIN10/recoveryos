...
export default function Dashboard() {
  ...
  const load = async (token = authToken, requestedPage = incidentPage.page, filters = { search, statusFilter, reviewOnly }) => {
    ...
    try {
      ...
      const [incidentResponse, batchResponse, policyResponse, operationsResponse, revenueResponse, experimentResponse, revenueOperationsResponse] = await Promise.all([apiFetch(`/incidents?${incidentQuery}`, { cache: 'no-store' }, token), apiFetch('/batches', { cache: 'no-store' }, token), apiFetch('/policies', { cache: 'no-store' }, token), apiFetch('/operations', { cache: 'no-store' }, token), apiFetch('/revenue/incidents?page=1&pageSize=100', { cache: 'no-store' }, token), apiFetch('/revenue/experiments', { cache: 'no-store' }, token), apiFetch('/revenue/operations', { cache: 'no-store' }, token)]);
      if ([incidentResponse, batchResponse, policyResponse, operationsResponse, revenueResponse, experimentResponse, revenueOperationsResponse].some(response => response.status === 401 || response.status === 403)) { setError('AUTH_REQUIRED'); return; }
      if (!incidentResponse.ok || !batchResponse.ok || !policyResponse.ok || !operationsResponse.ok || !revenueResponse.ok || !experimentResponse.ok || !revenueOperationsResponse.ok) throw new Error('API unavailable');
      const [incidentData, batchData, policyData, operationsData, revenueData, experimentData, revenueOperationsData] = await Promise.all([incidentResponse.json(), batchResponse.json(), policyResponse.json(), operationsResponse.json(), revenueResponse.json(), experimentResponse.json(), revenueOperationsResponse.json()]);
  ...
  return <main>
    <header><div><p className="eyebrow">RECOVERYOS · AI REVENUE RECOVERY</p><h1>Revenue recovery command center</h1><p className="subtle">Detect revenue at risk. Let AI propose a grounded playbook. Let policy authorize. Measure the attributed outcome.</p></div><div className="simulation">● CONTROLLED SIMULATION / TEST MODE</div></header>
...
function RevenueWorkspace({ configuration, incidents, detail, experiments, selectedExperiment, run, busy, error, onRun, onOpen, onSelectExperiment, onReview }: { configuration: RevenueConfiguration | null; incidents: RevenueIncident[]; detail: RevenueIncident | null; experiments: RevenueExperiment[]; selectedExperiment: RevenueExperiment | null; run: RevenueDemoRun | null; busy: boolean; error: string; onRun: () => void; onOpen: (id: string) => void; onSelectExperiment: (experiment: RevenueExperiment) => void; onReview: (approved: boolean) => void }) {
  const metrics = selectedExperiment?.metrics;
  const baseline = selectedExperiment?.baseline;
  const duplicateCount = run ? Object.values(run.duplicateReplayVerified).filter(Boolean).length : 0;
  return <section className="revenue-workspace">
    <section className="panel revenue-hero">
      <div><p className="eyebrow">INBOUND REVENUE · CONTROLLED EXPERIMENT</p><h2>Recover failed customer payments</h2><p>Groq reasons across a persisted event timeline and proposes a bounded playbook. Policy independently authorizes only the first safe step. A later captured-payment event is required before revenue is attributed.</p></div>
      <div className="revenue-hero-action"><span className={`badge ${configuration?.ready ? 'good' : 'warn'}`}>{configuration?.ready ? 'READY' : 'NOT READY'}</span><button className="approve" disabled={!configuration?.ready || busy} onClick={onRun}>{busy ? 'Running 8 AI analyses and durable actions…' : 'Run inbound revenue experiment'}</button><small>8 fixed cases · no-action baseline · conservative rules baseline · AI + policy</small></div>
      {error && <div className="demo-error">{error}</div>}
    </section>
    <section className="metrics revenue-metrics"><Metric label="Revenue at risk" value={money(metrics?.valueAtRiskPaise ?? 0)} /><Metric label="Revenue recovered" value={money(metrics?.recoveredValuePaise ?? 0)} accent="green" /><Metric label="Recovery rate" value={`${((metrics?.recoveryRate ?? 0) * 100).toFixed(1)}%`} accent="green" /><Metric label="Recovered payments" value={String(metrics?.recoveredIncidentCount ?? 0)} /><Metric label="Human gates" value={String(metrics?.manualReviewRequired ?? 0)} accent="orange" /><Metric label="Unsafe actions blocked" value={String(metrics?.unsafeActionsPrevented ?? 0)} /><Metric label="Evidence completeness" value={`${((metrics?.evidenceCompleteness ?? 0) * 100).toFixed(0)}%`} /><Metric label="Duplicate replays blocked" value={`${duplicateCount}/${run ? Object.keys(run.duplicateReplayVerified).length : 0}`} /></section>
    <section className="revenue-comparison">
      <div className="panel"><div className="panel-head"><div><p className="eyebrow">IMMUTABLE EVIDENCE</p><h2>Baseline comparison</h2></div>{selectedExperiment && <span className="badge good">FROZEN</span>}</div>
        {selectedExperiment ? <><div className="baseline-grid"><article><span>No recovery</span><strong>{money(baseline?.noAction.recoveredValuePaise ?? 0)}</strong><small>Observed without an intervention</small></article><article><span>Rules only</span><strong>{money(baseline?.rulesOnly.recoveredValuePaise ?? 0)}</strong><small>Conservative static eligibility</small></article><article className="winner"><span>AI + policy</span><strong>{money(baseline?.aiPolicy.recoveredValuePaise ?? 0)}</strong><small>Incremental vs rules {money(baseline?.aiPolicy.incrementalVsRulesPaise ?? 0)}</small></article></div><p className="metric-note">{baseline?.disclaimer}</p><div className="experiment-meta"><span>Policy <strong>{selectedExperiment.policyVersion}</strong></span><span>Model <strong>{selectedExperiment.modelRef}</strong></span><span>Prompt <strong>{selectedExperiment.promptVersion}</strong></span><span>Fingerprint <strong>{selectedExperiment.cohortFingerprint.slice(0, 12)}…</strong></span></div></> : <div className="empty">Run the controlled cohort to create immutable comparison evidence.</div>}
      </div>
      <div className="panel experiment-list"><div className="panel-head"><div><h2>Revenue experiments</h2><p>Snapshots never change when incidents change later</p></div></div>{experiments.length ? experiments.map(experiment => <button key={experiment.id} className={`batch-card ${selectedExperiment?.id === experiment.id ? 'selected' : ''}`} onClick={() => onSelectExperiment(experiment)}><span><strong>{experiment.name}</strong><small>{new Date(experiment.completedAt).toLocaleString()} · {experiment.cohortSize} cases</small></span><strong>{money(experiment.metrics.recoveredValuePaise)}</strong></button>) : <div className="empty">No revenue experiments yet.</div>}</div>
    </section>
    <section className="content revenue-incidents"><div className="panel list"><div className="panel-head"><div><h2>Inbound payment incidents</h2><p>{incidents.length} timeline-grounded decisions</p></div></div><div className="rows">{incidents.map(incident => <button className="row" key={incident.id} onClick={() => onOpen(incident.id)}><span><strong>{incident.sourcePaymentId}</strong><small>{incident.failureCode} · {incident.failureDescription}</small></span><span className={`badge ${tone(incident.status)}`}>{incident.status.replaceAll('_', ' ')}</span><strong>{money(incident.amountPaise)}</strong></button>)}{!incidents.length && <div className="empty">Run the inbound cohort to populate evidence.</div>}</div></div><aside className="panel detail">{detail ? <RevenueIncidentDetail detail={detail} onReview={onReview} /> : <div className="empty"><h2>Select a failed payment</h2><p>Inspect the cited timeline evidence, proposed playbook, policy boundary, attributed action and audit trail.</p></div>}</aside></section>
  </section>;
}

function RevenueIncidentDetail({ detail, onReview }: { detail: RevenueIncident; onReview: (approved: boolean) => void }) {
  ...
  return <><div className="panel-head"><div><p className="eyebrow">INBOUND PAYMENT INCIDENT</p><h2>{detail.sourcePaymentId}</h2></div><span className={`badge ${tone(detail.status)}`}>{detail.status.replaceAll('_', ' ')}</span></div><div className="facts"><span>Revenue at risk<strong>{money(detail.amountPaise)}</strong></span><span>Payment method<strong>{detail.paymentMethod}</strong></span><span>Failure<strong>{detail.failureCode}</strong></span><span>Contact consent<strong>{detail.consentToContact ? 'PRESENT' : 'ABSENT'}</strong></span></div>
    {proposal && <section className="evidence"><div className="evidence-head"><p className="eyebrow">AI TIMELINE DIAGNOSIS</p><span className="badge neutral">{analysis.modelRef}</span></div><strong>{proposal.category.replaceAll('_', ' ')} → {proposal.recommendedAction.replaceAll('_', ' ')}</strong><p>{proposal.diagnosis}</p><div className="cited-evidence">{proposal.evidence.map(item => <div key={`${item.eventId}-${item.fact}`}><code>{item.eventId}</code><span>{item.fact}</span></div>)}</div><small>Confidence {Math.round(proposal.confidence * 100)}% · Timeline {analysis.timelineDigest.slice(0, 12)}… · Prompt {analysis.promptVersion}</small></section>}
    {proposal && <section className="playbook"><p className="eyebrow">PROPOSED BOUNDED PLAYBOOK</p>{proposal.playbook.map(step => <div key={`${step.order}-${step.action}`}><span>{step.order}</span><p><strong>{step.action.replaceAll('_', ' ')}</strong><small>{step.delayMinutes} minute delay · {step.requiresHuman ? 'human required' : 'automated if policy allows'}</small>{step.rationale}</p></div>)}</section>}
    ...
    {detail.status === 'APPROVAL_REQUIRED' && <div className="actions"><button className="approve" onClick={() => onReview(true)}>Approve bounded first step</button><button onClick={() => onReview(false)}>Reject</button></div>}
    ...
    <section className="timeline"><p className="eyebrow">AUDIT TIMELINE</p>{detail.auditEvents?.map(event => <div className="event" key={event.id}><i></i><div><strong>{event.eventType.replaceAll('_', ' ')}</strong><p>{event.rationale}</p><small>{new Date(event.createdAt).toLocaleString()}</small></div></div>)}</section></>;
}
...
function IncidentDetail({ detail, onReview, onRemediate }: { detail: Detail; onReview: (approved: boolean) => void; onRemediate: (beneficiaryRef: string, note: string) => void }) {
  ...
  return <><div className="panel-head"><div><p className="eyebrow">PAYOUT INCIDENT DETAILS</p><h2>{detail.razorpayPayoutId}</h2></div><span className={`badge ${tone(detail.status)}`}>{detail.status.replaceAll('_', ' ')}</span></div><div className="facts"><span>Amount <strong>{money(detail.amountPaise)}</strong></span><span>Attempts <strong>{detail.attempts}</strong></span><span>Reason <strong>{detail.currentReason || '—'}</strong></span></div>{analysis && <section className="evidence"><div className="evidence-head"><p className="eyebrow">{deterministic ? 'DETERMINISTIC ADVISORY' : 'AI ADVISORY'}</p><span className="badge neutral">{analysisRecord?.modelRef}</span></div><strong>{analysis.category.replaceAll('_', ' ')}</strong><p>{analysis.evidenceSummary}</p><small>Confidence {Math.round(analysis.confidence * 100)}% · Proposed {analysis.recommendedAction} · Prompt {analysisRecord?.promptVersion}</small></section>}{decision && <section className="evidence policy"><p className="eyebrow">POLICY DECISION</p><strong>{decision.finalDecision.replaceAll('_', ' ')}</strong><p>{decision.reasonsJson.join(' ')}</p></section>}
    {detail.status === 'APPROVAL_REQUIRED' && <div className="actions"><button className="approve" onClick={() => onReview(true)}>Approve bounded retry</button><button onClick={() => onReview(false)}>Reject</button></div>}
    {detail.status === 'ESCALATE' && <section className="remediation"><p className="eyebrow">REMEDIATION REQUIRED · RETRY IS BLOCKED</p><p>An escalation cannot be approved directly. Record a validated replacement beneficiary; a separate actor must then approve the new retry task.</p><input value={beneficiaryRef} onChange={event => setBeneficiaryRef(event.target.value)} placeholder="Validated replacement beneficiary reference" /><textarea value={remediationNote} onChange={event => setRemediationNote(event.target.value)} placeholder="Verification evidence and remediation note" /><div className="actions compact"><button className="approve" disabled={beneficiaryRef.trim().length < 3 || remediationNote.trim().length < 10} onClick={() => onRemediate(beneficiaryRef, remediationNote)}>Record remediation</button><button onClick={() => onReview(false)}>Reject and stop</button></div></section>}
    <section className="timeline"><p className="eyebrow">AUDIT TIMELINE</p>{detail.auditEvents.map(event => <div className="event" key={event.id}><i></i><div><strong>{event.eventType.replaceAll('_', ' ')}</strong><p>{event.rationale}</p><small>{new Date(event.createdAt).toLocaleString()}</small></div></div>)}</section></>;
}
